...
async function extractEpisodes(url) {
    try {
        if(url.includes('movie')) {
            ...
            const movie = [
                { href: `https://vidlink.pro/movie/${movieId}`, number: 1, title: "Full Movie" }
            ];
            ...
            for (const season of showData.seasons) {
                ...
                if (seasonData.episodes && seasonData.episodes.length) {
                    const episodes = seasonData.episodes.map(episode => ({
                        href: `https://vidlink.pro/tv/${showId}/${seasonNumber}/${episode.episode_number}`,
...
async function extractStreamUrl(url) {
    try {
        const streams = await networkFetch(url, 7, {}, "/api/b/");

        console.log("Vidlink streams: " + JSON.stringify(streams));
        console.log("Vidlink streams: " + streams.requests.find(url => url.includes('/api/b/')));

        if (streams.requests && streams.requests.length > 0) {
            const apiUrl = streams.requests.find(url => url.includes('/api/b/')) || "";
            ...
            const subtitles = data.stream.captions.find(track => track.language.includes('English'))?.url;
