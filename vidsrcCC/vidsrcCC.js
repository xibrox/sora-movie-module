...
async function extractEpisodes(url) {
    try {
        if(url.includes('movie')) {
            ...
            const movie = [
                { href: `https://vidsrc.cc/v2/embed/movie/${movieId}`, number: 1, title: "Full Movie" }
            ];
...
async function extractStreamUrl(url) {
    try {
        ...
        console.log("VidSrc streams: " + JSON.stringify(serverList));
        console.log("VidSrc streams: " + serverList.requests.find(url => url.includes('servers')));

        if (serverList.requests && serverList.requests.length > 0) {
            const apiUrl = serverList.requests.find(url => url.includes('servers')) || "";
            ...
            for (const server of servers) {
                ...
                const subtitles2 = source.data?.subtitles?.find(track => track.label.includes('English') && track.kind === "captions")?.file;

                console.log("Stream URL: " + streamUrl);
                console.log("Subtitles: " + subtitles2);
