...
async function extractStreamUrl(url) {
    try {
        ...
        if (streams.requests && streams.requests.length > 0) {
            const matches = streams.requests.filter(url => url.includes('https://111movies.com/to') && !url.includes('ne') && !url.includes('sr'));
            const apiUrl = matches[0] || "";
            ...
            const response = await soraFetch(apiUrl, { method: "POST" });
